/**
 * Bulk Task Voice Service
 *
 * Continuous voice recording with pause detection for bulk task dictation.
 * Records multiple task lines, then parses them into structured tasks.
 * Based on BulkIngredientVoice but adapted for task-specific patterns.
 */
...
export const PAUSE_PRESETS = {
  ...
  FAST: {
    ...
    pauseDuration: TIMEOUTS.VOICE_PAUSE_SHORT || 1000,
  },
  /** Default balanced setting */
  NORMAL: {
    name: 'Normal',
    description: 'Balanced setting (1.5 second pause)',
    pauseDuration: TIMEOUTS.VOICE_PAUSE_DETECTION || 1500,
  },
  /** For slow/thoughtful speakers */
  SLOW: {
    name: 'Slow',
    description: 'For slow speakers (2.5 second pause)',
    pauseDuration: TIMEOUTS.VOICE_PAUSE_LONG || 2500,
  },
};

/**
 * BulkTaskVoice class
 * Manages continuous speech recognition with pause detection for task dictation
 */
export class BulkTaskVoice {
  /**
   * Create a new BulkTaskVoice instance
   ...
   * Splits on sentence-ending punctuation for task separation
   */
  processCurrentLine() {
    if (this.currentLine.trim()) {
      // Split on punctuation that indicates task boundaries
      const taskSegments = this.currentLine
        .split(/[.!?;]|,\s*(?=pour\s)/i)
        .map(s => s.trim())
        .filter(s => s.length > 2);
      ...
      for (const segment of taskSegments) {
        if (segment && !this.lines.includes(segment)) {
          this.lines.push(segment);
        }
      }

      // If no punctuation found, add the whole line
      if (taskSegments.length === 0 && this.currentLine.trim().length > 2) {
